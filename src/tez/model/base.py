...
class TezModel(ABC):
    def train(self, train_dataset, **kwargs):
        self.m.fit(train_dataset.data, train_dataset.targets, **kwargs)
    ...
    def evaluate(self, validation_dataset, **kwargs) -> EvaluationResponse:
        if validation_dataset.metric_name in (
            ...
            probas = self.m.predict_proba(validation_dataset.data)
            preds = self.m.predict(validation_dataset.data)
