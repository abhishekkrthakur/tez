...
if __name__ == "__main__":
    ...
    train_dataset = BERTDataset(text=df_train.text.values, target=df_train.category.values)

    valid_dataset = BERTDataset(text=df_valid.text.values, target=df_valid.category.values)

    n_train_steps = int(len(df_train) / 32 * 10)
    model = BERTBaseUncased(num_train_steps=n_train_steps, num_classes=dfx.category.nunique())
